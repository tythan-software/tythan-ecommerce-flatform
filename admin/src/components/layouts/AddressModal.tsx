...
import { CreateOrUpdateUserAddress, UserAddress } from "@/types/User";
import Input, { Label } from "../partials/Input";
...
const AddressModal = ({ 
  ...
  const [addressForm, setAddressForm] = useState<CreateOrUpdateUserAddress>({
    label: "",
    street: "",
    city: "",
    state: "",
  ...
  const openAddressForm = (address: UserAddress | null = null) => {
    if (address) {
  ...
  const handleSaveAddress = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingAddress) {
        ...
        response = await userService.updateUserAddress(
          userId,
          editingAddress._id,
          addressForm
        );
        ...
        // Add new address
        response = await userService.createUserAddress(userId, addressForm);
      }
  ...
  const handleDeleteAddress = async (addressId: string) => {
    ...
    try {
      setLoading(true);
      const response = await userService.deleteUserAddress(userId, addressId);
  ...
  const handleSetDefaultAddress = async (addressId: string) => {
    try {
      setLoading(true);
      const response = await userService.setUserDefaultAddress(userId, addressId);
