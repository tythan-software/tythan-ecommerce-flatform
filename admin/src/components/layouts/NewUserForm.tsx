...
import { CreateOrUpdateUser, UserAddress } from "@/types/User";
import userService from "@/services/userService";
import Input, { Label, UploadFile } from "../partials/Input";
...
interface Props {
  ...
  close: () => void;
  selectedUser: any; // You can replace 'any' with a proper User type if available
  getUsersList: () => void;
  token: string;
  isReadOnly?: boolean; // New prop to indicate read-only mode
}
...
const NewUserForm = ({
  ...
  const uploadAvatar = async () => {
    ...
    try {
      const response = await fileService.uploadAvatar(avatarFile);
