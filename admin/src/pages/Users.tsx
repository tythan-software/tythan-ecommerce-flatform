...
import NewUserForm from "@/components/layouts/NewUserForm";
import User from "@/types/User";

const Users = () => {
  const dispatch = useDispatch();
  const { user: currentUser, token } = useSelector((state: any) => state.auth);
  ...
  const handleViewUser = (user: User) => {
    if (isAdmin) {
      ...
      toast.error("Read-only view - Only administrators can edit users");
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 p-3 sm:p-4 lg:p-6">
      <div className="max-w-7xl mx-auto">
                      ...
                      <tr key={user._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                          ...
                          {user.addresses && user.addresses.length > 1 ? (
                              ...
                              {user.addresses
                                ...
                                .map((addr) => (
                                  <div key={addr._id || addr.label}>
                                    ...
                                    <div>
                                      {compact([addr.city, addr.country]).join(", ")}
                                    </div>
                                    {addr.phone && (
                            ...
                            <div>
                              {compact([user.addresses[0].city, user.addresses[0].country]).join(", ")}
        ...
        <NewUserForm
          ...
          token={token}
          isReadOnly={!isAdmin && selectedUser != null}
        />
      </div>
    </div>
  );
